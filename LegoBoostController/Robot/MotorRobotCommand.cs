...
namespace LegoBoostController.Robot
{
    ...
    {
        public async Task RunAsync(IHubController controller, string commandText, string clockwiseKeyword, Motor motor)
        {
            ...
            if (m.Groups.Count == 3)
            {
                ...
                var port = motor.Name == "External" ? controller.GetPortIdsByDeviceType(IOTypes.ExternalMotor).First() : motor.Code;
